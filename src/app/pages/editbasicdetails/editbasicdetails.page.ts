...
@Component({
  selector: 'app-editbasicdetails',
  templateUrl: './editbasicdetails.page.html',
  styleUrls: ['./editbasicdetails.page.scss'],
})
export class EditbasicdetailsPage implements OnInit {
  ...
  updateDetails() {
    ...
    this.api.post('users/edit_profile', param).then((data: any) => {
      ...
      console.log(data);
      this.getProfile();
      this.util.showToast(this.util.translate('User details updated successfully'), 'dark', 'bottom');
      this.back();
    }, error => {
      this.util.hide();
      console.log(error);
